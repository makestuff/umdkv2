	...
	struct FLContext *handle, uint32 address, bool enabled, const char **error)
{
	...
	fStatus = flWriteChannelAsync(handle, BRK0_REG, 1, &byte, error);
	...
	fStatus = flWriteChannelAsync(handle, BRK2_REG, 1, &byte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "umdkSetBreakpointAddress()");
	address >>= 8;
	byte = address & 0x000000FF;
	fStatus = flWriteChannelAsync(handle, BRK1_REG, 1, &byte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "umdkSetBreakpointAddress()");
	address >>= 8;
	if ( enabled ) {
	...
	fStatus = flWriteChannel(handle, BRK0_REG, 1, &byte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "umdkSetBreakpointAddress()");
	retVal = UMDK_SUCCESS;
cleanup:
	return retVal;
	...
	struct FLContext *handle, bool enabledFlag, const char **error)
{
	...
	// Read it...
	fStatus = flReadChannel(handle, BRK0_REG, 1, &byte, error);
...
static UMDKStatus remoteAcquire(struct UMDK *handle, bool isInterrupted, const char **error) {
	...
	fStatus = flWriteChannelAsync(handle->fpga, FLAGS_REG, 1, &handle->flagByte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "remoteAcquire()");
	fStatus = flReadChannel(handle->fpga, FLAGS_REG, 1, &handle->flagByte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "remoteAcquire()");
	...
	struct FLContext *handle, uint32 address, const char **error)
{
	...
	byte = address & 0x000000FF;
	fStatus = flWriteChannelAsync(handle, ADDR2_REG, 1, &byte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "setAddress()");

	// Write middle byte:
	address >>= 8;
	byte = address & 0x000000FF;
	fStatus = flWriteChannelAsync(handle, ADDR1_REG, 1, &byte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "setAddress()");

	// Write MSB
	address >>= 8;
	byte = address & 0x000000FF;
	fStatus = flWriteChannel(handle, ADDR0_REG, 1, &byte, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "setAddress()");
	retVal = UMDK_SUCCESS;
cleanup:
	return retVal;
	...
	struct FLContext *handle, uint32 address, uint32 count, const uint8 *data, const char **error)
{
	...
	fStatus = flReadChannel(handle, FLAGS_REG, 1, &flags, error);
	...
	fStatus = flWriteChannel(handle, DATA_REG, count, data, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "writeMemDirect()");
	retVal = UMDK_SUCCESS;
cleanup:
	return retVal;
	...
	struct FLContext *handle, uint32 address, uint32 count, uint8 *buf, const char **error)
{
	...
	fStatus = flReadChannel(handle, DATA_REG, count, buf, error);
	CHECK_STATUS(fStatus, UMDK_FPGALINK_ERR, cleanup, "readMemDirect()");
	retVal = UMDK_SUCCESS;
cleanup:
	return retVal;
...
static void memWrite(struct I68K *self, uint32 address, uint32 length, const uint8 *buf) {
	...
	if ( address < 0x800000 ) {
		...
		if ( flWriteChannel(umdk->fpga, FLAGS_REG, 1, &umdk->flagByte, &error) ) {
			goto cleanup;
		}
	...
	uint32 brkAddress, IsIntFunc isInterrupted)
{
	...
	if ( flWriteChannel(fpga, FLAGS_REG, 1, &umdk->flagByte, &error) ) {
		goto cleanup;
	}
