...
mkdir menu
cp sample/joytest/src/main.c menu/

patch menu/main.c <<EOF
--- menu/main.old	2014-04-21 18:02:16.527268111 +0100
+++ menu/main.new	2014-04-21 18:03:32.883450892 +0100
@@ -16 +16 @@
-    VDP_setPalette(PAL0, font_lib.palette);
+    //VDP_setPalette(PAL0, font_lib.palette);
EOF

cat > menu/Makefile <<EOF
GCC_HOME := \$(HOME)/x-tools/m68k-unknown-elf
TOOLS = \$(GCC_HOME)/bin/m68k-unknown-elf
#OPTS := -O1 -fomit-frame-pointer
OPTS := -g
CC_FLAGS := -m68000 -Wall \$(OPTS) -fno-builtin-memset -fno-builtin-memcpy -I ../inc -I . -I ../res -c
...
menu.elf: main.o sega.o
	\$(TOOLS)-gcc -n -T ../md.ld -e 0x200 -nostdlib sega.o main.o ../src/libmd.a \$(GCC_HOME)/lib/gcc/m68k-unknown-elf/4.3.4/libgcc.a -o menu.elf

sega.o: ../src/boot/sega.s ../src/boot/rom_head.c
	@mkdir -p out
	\$(TOOLS)-gcc \$(CC_FLAGS) ../src/boot/rom_head.c -o out/rom_head.bin
	\$(TOOLS)-gcc \$(CC_FLAGS) ../src/boot/sega.s -o \$@
